...
contract("ERC677", async accounts => {
  ...
  beforeEach(async () => {
    erc677 = await ERC677.new(accounts[0], web3.utils.toBN("201000000000000000000000000000"), "CSD", "CSD");
  ...
  it("should emit received data", async () => {
    ...
    const { tx } = await erc677.transferAndCall(to, amount, data);

    expectEvent.inTransaction(tx, receiver, "Test", { from, amount, data });
  });
});